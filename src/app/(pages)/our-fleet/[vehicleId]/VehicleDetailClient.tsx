...
import { getVehicleSlug, resolveColorVariantImage } from "@/data/fleet";
...
const VehicleDetailClient = () => {
  ...
  const selectedCar: Car | undefined = vehicleData
    ? {
...
const handleBookNow = () => {
  ...
  const getColorOptions = () => {
    if (vehicleData?.colorVariants?.length) {
      return vehicleData.colorVariants.map((variant) => ({
        id: variant.id,
        name: variant.name,
        color: variant.color ?? "#808080",
        image: resolveColorVariantImage(variant),
      }));
    }
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    >
                      <Image
                        key={selectedColor}
                        src={getFleetImageSrc(currentColorImage)}
